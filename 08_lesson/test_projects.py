...
def test_create_project_negative_empty_title(api_client):
    """Негативный тест: Создание проекта с пустым заголовком (отсутствие обязательного поля)"""
...
def test_update_project_positive(api_client, temp_project):
    ...
    response = api_client.update_project(project_id=temp_project, title=new_title)
...
def test_update_project_negative_non_existent(api_client):
    """Негативный тест: Попытка обновить параметры у несуществующего проекта"""
    invalid_id = "00000000-0000-0000-0000-000000000000"
    response = api_client.update_project(project_id=invalid_id, title="New Title")
